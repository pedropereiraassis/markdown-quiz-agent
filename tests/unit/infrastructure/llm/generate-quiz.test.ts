...
function createMarkdownSource(overrides: Partial<MarkdownSource> = {}): MarkdownSource {
  const markdown = overrides.markdown ?? '# Quiz Source\n\nFact one.\n\nFact two.';
...
describe('createOpenRouterQuizGenerator', () => {
  ...
  it('fails fast when the configured model id is missing, empty, or openrouter/auto', () => {
    ...
    expect(() =>
      createOpenRouterQuizGenerator(createConfig({ model: 'openrouter/auto' }), {
        createModel: () => {
          throw new Error('should not create model');
        },
      }),
  ...
  it('throws QuizGenerationProviderError when the LLM call exceeds the timeout', async () => {
    ...
    const invoke = vi.fn(async (_prompt: string, options?: { signal?: AbortSignal }) => {
    ...
    const assertion = expect(generatePromise).rejects.toBeInstanceOf(QuizGenerationProviderError);
  ...
  it('passes the configured pinned model id into the provider factory', () => {
    ...
    createOpenRouterQuizGenerator(createConfig({ model: 'anthropic/claude-4-sonnet' }), {
      createModel,
    });
  ...
  describe('logging', () => {
    ...
    it('emits quiz_generation_schema_failure when validation fails', async () => {
      ...
      expect(logger.error).toHaveBeenCalledWith('quiz_generation_schema_failure', {
        generationAttempt: 1,
        providerModel: 'openai/gpt-4.1-mini',
