import { FETCH_LIMITS, QUIZ_OPTION_COUNT, QUIZ_QUESTION_LIMITS } from '../../config/constants.js';
...
export function buildQuizPrompt(input: BuildQuizPromptInput): string {
  const questionCount = validateQuestionCount(input.questionCount ?? QUIZ_QUESTION_LIMITS.min);
  const markdown = input.source.markdown.trim();

  if (markdown.length === 0) {
    throw new QuizGenerationInputError('Quiz generation requires non-empty bounded Markdown');
  }
...
function validateQuestionCount(questionCount: number): number {
  if (!Number.isInteger(questionCount)) {
    throw new QuizGenerationInputError('Quiz generation questionCount must be an integer');
  }

  if (questionCount < QUIZ_QUESTION_LIMITS.min || questionCount > QUIZ_QUESTION_LIMITS.max) {
    throw new QuizGenerationInputError(
