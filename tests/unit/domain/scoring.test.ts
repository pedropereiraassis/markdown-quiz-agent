...
describe('scoring', () => {
  ...
  it('awards partial credit for multiple-answer questions and penalizes wrong extras', () => {
    expect(scoreMultipleAnswerQuestion(['q2-a', 'q2-b'], ['q2-a'])).toBe(2);
    expect(scoreMultipleAnswerQuestion(['q2-a', 'q2-b'], ['q2-a', 'q2-c'])).toBe(0);
    expect(scoreMultipleAnswerQuestion(['q2-a', 'q2-b', 'q2-c'], ['q2-a', 'q2-b'])).toBe(
      8 / 3,
    );
  });

  it('clamps multiple-answer scores at 0 when wrong extras outweigh correct selections', () => {
    expect(
      scoreMultipleAnswerQuestion(['q2-a', 'q2-b', 'q2-c'], ['q2-d', 'q2-e', 'q2-a']),
    ).toBe(0);
  });
  ...
  it('calculates the weighted final score without presentation rounding', () => {
    ...
    expect(result.questionResults.map((entry) => entry.pointsAwarded)).toEqual([4, 2, 0, 8 / 3, 4]);

    const expectedFinalScore =
      (4 * 1 + 2 * 1.1 + 0 * 1.21 + (8 / 3) * 1.3310000000000004 + 4 * 1.4641000000000006) /
