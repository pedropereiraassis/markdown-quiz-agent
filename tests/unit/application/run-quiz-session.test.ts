...
describe('createRunQuizSession', () => {
  it('returns a prepared quiz state from bounded source and validated quiz inputs', async () => {
    ...
    expect(prepared.questions).toEqual(
      quiz.questions.map(({ correctOptionIds: _correctOptionIds, ...question }) => question),
    );
    expect(prepared.questions[0]).not.toHaveProperty('correctOptionIds');
    expect(saveSession).not.toHaveBeenCalled();
  });

  it('session token does not encode correct option ids', async () => {
    const source = createMarkdownSource();
    const quiz = createQuiz();
    const runQuizSession = createRunQuizSession({
    ...
    const decoded = JSON.parse(
      Buffer.from(prepared.sessionToken, 'base64url').toString('utf8'),
    ) as Record<string, unknown>;

    expect(decoded).not.toHaveProperty('quiz');
    expect(Array.isArray(decoded['questions'])).toBe(true);
    for (const q of decoded['questions'] as unknown[]) {
  ...
  it('cannot complete the same session twice', async () => {
    ...
    await runQuizSession.complete({ answers, sessionToken: prepared.sessionToken });
  ...
  describe('logging', () => {
    ...
    it('emits source_fetch_failed on markdown ingestion errors', async () => {
      ...
      await expect(runQuizSession.prepare('https://example.com/guide.md')).rejects.toBeInstanceOf(
        RunQuizSessionError,
      );
    ...
    it('emits complete_failed on persistence errors', async () => {
      ...
      const runQuizSession = createRunQuizSession({
        ...
        quizGenerator: { generate: vi.fn().mockResolvedValue(quiz) },
        quizSessionRepository: {
          saveSession: vi.fn().mockRejectedValue(
  ...
  it.each([
    [
      ...
      async () => {
        ...
        await expect(runQuizSession.prepare('ftp://example.com/file.md')).rejects.toMatchObject({
    ...
    [
      ...
      async () => {
        ...
        await expect(runQuizSession.prepare(source.originalUrl)).rejects.toMatchObject({
        ...
        const runQuizSession = createRunQuizSession({
          ...
          quizSessionRepository: {
            saveSession: vi
              .fn()
              .mockRejectedValue(
                new QuizSessionPersistenceError('Failed to persist quiz session'),
              ),
          },
        });
        const prepared = await runQuizSession.prepare(source.originalUrl);
