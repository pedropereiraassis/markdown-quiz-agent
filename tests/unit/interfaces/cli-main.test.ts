...
describe("CLI main helpers", () => {
  it.each([
    [
      "invalid_source_url",
      "Enter a valid absolute http:// or https:// Markdown URL.",
    ],
    [
      "source_unavailable",
  ...
  it("formats startup configuration failures without leaking internal detail", () => {
    expect(
      formatStartupError(new QuizGenerationConfigError("invalid model")),
    ).toBe(
    ...
    expect(
      formatStartupError(
        new Error(
          "Invalid environment configuration: OPENROUTER_API_KEY: Invalid input",
        ),
      ),
    ).toBe(
  ...
  it("renders a generic error for unexpected CLI failures", async () => {
    ...
    expect(progressCalls[0]?.message).toBe(
  ...
  it("retries the source prompt after a recoverable prepare failure", async () => {
    ...
    expect(progressCalls[1]?.progress.success).toHaveBeenCalledWith(
      "Prepared 1 questions.",
    );
    expect(output.info).toHaveBeenCalledWith(
      "Paste another Markdown URL to try another source.",
    );
