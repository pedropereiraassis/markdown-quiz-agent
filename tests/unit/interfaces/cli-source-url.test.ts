...
describe('CLI source URL prompt', () => {
  it.each([
    ...
    ['ftp://example.com/guide.md', 'Enter a valid absolute http:// or https:// URL.'],
  ...
  it('returns the trimmed URL from the text prompt', async () => {
    const promptText = vi.fn().mockResolvedValue('  https://example.com/guide.md  ');

    await expect(promptSourceUrl({ promptText })).resolves.toBe('https://example.com/guide.md');
