...
describe('fetchMarkdown integration', () => {
  it('runs normalization, fetch, truncation, and metadata shaping in one call', async () => {
    ...
    expect(result).toMatchObject({
      originalUrl: 'https://github.com/openai/docs/blob/main/guides/example.md?raw=1#intro',
      normalizedUrl: 'https://raw.githubusercontent.com/openai/docs/main/guides/example.md',
      title: 'Example Title',
      wasTruncated: true,
    });
    expect(result.markdown.length).toBeLessThanOrEqual(260);
    expect(result.markdown.startsWith('# Example Title\n\nIntro paragraph.')).toBe(true);
    expect(result.markdown.includes('## Section One')).toBe(true);
    expect(result.originalCharacters).toBeGreaterThan(result.retainedCharacters);
